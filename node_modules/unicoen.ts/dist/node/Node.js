...
class Node {
    constructor({ className, members = null, isAbstract = false, superClassName = null, children = null, }) {
        this.className = '';
        ...
        if (members) {
            for (let [name, type] of members) {
                ...
                this.members.push(new Field_1.default(name, type));
            }
        }
        if (children) {
    ...
    makeImportText() {
        ...
        for (const field of this.members) {
            for (const type of field.type.split('|').map((t) => t.trim())) {
    ...
    makeToStringText() {
        ...
        const s1 = `${this.indent(1)}`;
        const s2 = `${this.indent(2)}`;
        let ret = `${s1}public toString(): string {\n`;
    ...
    makeToEqualsText() {
        ...
        let ret = `${s1}public equals(obj: any): boolean {\n`;
        ...
        if (this.className !== 'UniNode') {
            for (const field of this.members) {
                ...
                const types = field.type.split('|').map((type) => type.trim());
                const eqaulsTypes = types.filter((value) => value !== 'string' && value !== 'number' && value !== 'boolean' && value !== 'any');
                const opeTypes = types.filter((value) => value === 'string' || value === 'number' || value === 'boolean' || value === 'any');
                if (2 <= types.length) {
    ...
    makeMergeText() {
        ...
        const s3 = `${this.indent(3)}`;
        const s4 = `${this.indent(4)}`;
        let ret = `${s1}public merge(that: ${this.className}) {\n`;
        if (this.superClassName) {
